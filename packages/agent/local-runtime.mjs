...
/** Hybrid / Docker-only, host-approved local operations. No credentials or host shell are forwarded. */
export function createLocalRuntime({
  ...
  if (workspace === path.parse(workspace).root || inside(workspace, ledger) || inside(ledger, workspace)) {
    throw Error('工作区与审批目录必须独立，禁止挂载根目录');
  }
  if (typeof dockerImage !== 'string' || !dockerImage || dockerImage.startsWith('-') || /\s/.test(dockerImage)) {
    throw Error('Docker 镜像配置无效');
  }
  ...
  function checkWorkspace() {
    for (let current = workspace; ; current = path.dirname(current)) {
      if (fs.lstatSync(current).isSymbolicLink()) throw Error('运行工作区不能经过符号链接');
  ...
  for (const name of fs.readdirSync(ledger)) {
    ...
    try {
      const record = JSON.parse(fs.readFileSync(path.join(ledger, name), 'utf8'));
      if (!validId(record.id) || name !== record.id + '.json' || record.requestHash !== hash(record.request)) continue;
      if (record.status === 'consumed') {
        record.status = 'unknown';
        record.error = '服务中断，执行结果未知；审批已消费，未自动重放';
        persist(record);
      } else records.set(record.id, record);
    } catch {
      /* A damaged approval is never executable. */
    }
  }

  async function probeDocker() {
    try {
      await execFileImpl('docker', ['version', '--format', '{{.Server.Version}}'], { timeout: 3000, maxBuffer: MAX_OUTPUT, windowsHide: true });
      await execFileImpl('docker', ['image', 'inspect', dockerImage], { timeout: 3000, maxBuffer: MAX_OUTPUT, windowsHide: true });
      return true;
    } catch {
      return false;
    }
  ...
  async function status() {
    ...
    // backend === 'docker'
    try {
      await execFileImpl('docker', ['version', '--format', '{{.Server.Version}}'], { timeout: 5000, maxBuffer: MAX_OUTPUT, windowsHide: true });
      await execFileImpl('docker', ['image', 'inspect', dockerImage], { timeout: 5000, maxBuffer: MAX_OUTPUT, windowsHide: true });
      return { available: true, backend: 'docker', dockerImage, workspaceRoot: workspace, network: 'none' };
    } catch (e) {
      return { available: false, backend: 'docker', dockerImage, workspaceRoot: workspace, network: 'none', status: 'blocked', error: 'Docker 或本地镜像未就绪：' + String(e.message || e) };
    }
  }
  ...
  function propose(input = {}) {
    ...
    if (input.kind === 'exec') {
      if (typeof input.command !== 'string' || !input.command.trim() || input.command.length > 8000 || input.command.includes('\0')) {
      ...
      if (input.kind === 'write') {
        if (typeof input.content !== 'string' || Buffer.byteLength(input.content) > 1024 * 1024) {
          throw Error('写入内容不能超过 1 MiB');
        }
        request.content = input.content;
      }
      if (input.kind === 'search') {
        if (typeof input.query !== 'string' || !input.query || input.query.length > 2000 || input.query.includes('\0')) {
          throw Error('搜索内容需为 1–2000 字符');
        }
    ...
    const record = {
      ...
      ...(typeof input.conversationId === 'string' ? { conversationId: input.conversationId.slice(0, 100) } : {}),
    };
    persist(record);
    return view(record);
  }
  ...
  function deny(id) {
    ...
    if (!record) throw Error('审批不存在');
    if (record.status !== 'pending') return view(record);
    const next = { ...record, status: 'denied', finishedAt: new Date().toISOString() };
    persist(next);
    return view(next);
  }
  ...
  async function executeNative(request, signal) {
    ...
    if (request.kind === 'read') {
      if (!fs.existsSync(targetPath)) {
        throw Error(`文件未找到: ${request.path}`);
      }
      const stat = fs.statSync(targetPath);
      if (stat.isDirectory()) {
        throw Error(`目标是目录，无法读取内容: ${request.path}`);
      }
    ...
    if (request.kind === 'write') {
      ...
      fs.writeFileSync(targetPath, request.content, 'utf8');
      return { stdout: '', stderr: '' };
    }

    if (request.kind === 'list') {
      if (!fs.existsSync(targetPath)) {
        throw Error(`目录未找到: ${request.path}`);
      }
      ...
      const lines = entries.map((entry) => {
        ...
        try {
          ...
          return `${typeChar}rw-r--r-- 1 zora zora ${stat.size} ${stat.mtime.toISOString().slice(0, 16)} ${entry.name}`;
        } catch {
          return `-rw-r--r-- 1 zora zora 0 1970-01-01T00:00 ${entry.name}`;
        }
      });
    ...
    if (request.kind === 'search') {
      const matches = [];
      function searchRecursive(currentDir, relPrefix) {
        if (!fs.existsSync(currentDir)) return;
        const entries = fs.readdirSync(currentDir, { withFileTypes: true });
        for (const entry of entries) {
          if (sensitive(entry.name)) continue;
          const full = path.join(currentDir, entry.name);
          const rel = relPrefix ? `${relPrefix}/${entry.name}` : entry.name;
          if (entry.isDirectory()) {
            searchRecursive(full, rel);
          } else if (entry.isFile()) {
            try {
              const text = fs.readFileSync(full, 'utf8');
              const lines = text.split(/\r?\n/);
              for (let i = 0; i < lines.length; i++) {
                if (lines[i].includes(request.query)) {
      ...
      searchRecursive(targetPath, request.path === '.' ? '' : request.path);
  ...
  async function approveOnce(id) {
    ...
    if (
      disk.status !== 'pending' ||
      disk.requestHash !== record.requestHash ||
      hash(disk.request) !== record.requestHash ||
      disk.request.workspaceRoot !== workspace ||
      disk.request.dockerImage !== dockerImage
    ) {
      throw Error('审批内容已改变，拒绝执行');
    }
    ...
    try {
      artifacts = createWorkspaceArtifacts({ workspaceRoot: workspace });
      before = new Map(artifacts.list().map((file) => [file.id, `${file.size}:${file.modifiedAt}`]));
    } catch {
      next.artifactWarning = '无法记录文件变化，完成后请查看素材库';
    }
    ...
    if (ready.backend === 'native') {
      ...
      if (request.kind === 'exec') args.push('sh', '-lc', request.command);
        ...
        args.push('sh', '-lc', 'grep -r -n -F -I -- "$2" "$1"; code=$?; if [ "$code" -eq 1 ]; then exit 0; fi; exit "$code"', 'zora-search', request.path, request.query);
      ...
      try {
        const result = await execFileImpl('docker', args, {
          ...
          windowsHide: true,
          ...(request.kind === 'write' ? { input: request.content } : {}),
        });
        Object.assign(next, {
          status: 'completed',
          stdout: String(result.stdout || '').slice(0, MAX_OUTPUT),
        ...
        try {
          await execFileImpl('docker', ['rm', '-f', container], { timeout: 5000, maxBuffer: MAX_OUTPUT, windowsHide: true });
        } catch {}
        Object.assign(next, {
          status: 'failed',
          error: String(e.message || e).slice(0, 4000),
    ...
    if (next.status === 'completed' && before) {
      try {
        next.artifacts = artifacts.list().filter((file) => before.get(file.id) !== `${file.size}:${file.modifiedAt}`);
      } catch {
  ...
  function deleteRecord(id) {
    ...
    if (['pending', 'consumed', 'running'].includes(record.status)) throw Error('请先处理审批或等待执行结束');
    persist({ ...record, deletedAt: new Date().toISOString() });
    return { ok: true, id };
  }
