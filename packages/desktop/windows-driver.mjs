...
const actions=new Set(['listWindows','launchJianying','readWindow','click','type','keys']);
const keys=new Set(['Enter','Escape','Tab','Backspace','Delete','Space','Up','Down','Left','Right','Home','End','PageUp','PageDown','Ctrl+A','Ctrl+Z','Ctrl+Y','Ctrl+S']);
...
export async function invokeDesktop(input={}, {execFileImpl=execute}={}){
 ...
 if(!['listWindows','launchJianying'].includes(input.action)){
  if(typeof input.windowId!=='string'||!/^\d{1,20}$/.test(input.windowId)||BigInt(input.windowId)===0n)return {ok:false,error:'必须指定已枚举的剪映窗口编号'};
 ...
 try{
  ...
  const result=await execFileImpl(executable,['-NoLogo','-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',script],{windowsHide:true,timeout:20000,maxBuffer:256*1024,encoding:'utf8',input:JSON.stringify(command)});
