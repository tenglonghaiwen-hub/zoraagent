...
export function getRouteCapabilities(model){
 ...
 if(model.family==='minimax')return [{operation:'generate',apiRoute:'/v1/videos',description:'t2v，无素材'}, {operation:'reference',apiRoute:'/v1/videos',description:'i2v/fl 使用 adaptive；ref 无图片或至少3张图'}];
 // H3 仅开放多参 reference；其他生成模式停用。
 return [
  ...(model.family==='minimax'?[{operation:'reference',apiRoute:'/v1/videos',requiresReferences:true,description:'reference_image + reference_video + reference_audio; videoMode=ref'}]:[]),
...
export function validateRouteSelection(input,model){
 ...
 const capability=getRouteCapabilities(model).find(r=>r.operation===operation&&(hasRoute?r.apiRoute===input.apiRoute:r.apiRoute===(model.family==='minimax'&&input.videoMode==='ref'?'/v1/videos':model.family==='qwen-image'&&operation==='reference'?'/v1/images/edits':model.route)));
