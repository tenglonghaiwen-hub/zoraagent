...
try{
 ...
 const color=s=>page.locator(s).evaluate(e=>getComputedStyle(e).backgroundColor);
 assert.equal(await color('.canvas-agent-rail'),'rgb(247, 247, 249)');assert.equal(await color('.canvas-agent-composer'),'rgb(255, 255, 255)');
 await page.locator('#canvas-agent-model-trigger').click();assert.equal(await color('#canvas-agent-model-menu'),'rgb(255, 255, 255)');await page.screenshot({path:path.join(output,'rail-day.png'),timeout:20000});await page.locator('#canvas-agent-input').click();
 await page.locator('#theme-toggle').click();assert.equal(await color('.canvas-agent-rail'),'rgb(20, 23, 30)');assert.equal(await color('.canvas-agent-composer'),'rgb(32, 36, 45)');await page.locator('#canvas-agent-model-trigger').click();assert.equal(await color('#canvas-agent-model-menu'),'rgb(32, 36, 45)');await page.screenshot({path:path.join(output,'rail-night.png'),timeout:20000});
 await page.reload();await canvas();assert.equal(await color('.canvas-agent-rail'),'rgb(20, 23, 30)');assert.equal(errors.length,0);console.log('PASS rail composer menus theme switch and reload');
