...
try{
 ...
 await app.evaluate(({app,session},dir)=>{app.setPath('downloads',dir);globalThis.downloadStates=[];session.defaultSession.on('will-download',(e,item)=>item.on('done',(event,state)=>globalThis.downloadStates.push({state,path:item.getSavePath()})));},output);
 await page.locator('[data-tab="settings"]').first().click();await page.getByRole('switch',{name:'完成后自动下载'}).check();assert.equal(await readStore('zora.autoDownload.v1'),1);assert.equal(await page.getByText('第三方 API',{exact:true}).count(),0);await page.locator('[data-tab="create"]').first().click();await canvas();
 ...
 const mediaRequests=[];await page.route('**/api/generate',async route=>{const payload=route.request().postDataJSON();mediaRequests.push(payload);await route.fulfill({json:{upstream:{url:payload.modelId==='grok-video-3'?'data:video/mp4;base64,AAAA':'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+jB1sAAAAASUVORK5CYII='}}});});
 await at('image-b').locator('.canvas-node-body').click();await at('image-b').locator('.node-run').click();await page.waitForFunction(()=>JSON.parse(localStorage.getItem('zora.canvasNodes.v1'))[1].runState==='complete');assert.match(mediaRequests[0].prompt,/本地验收模拟/);assert.match(mediaRequests[0].prompt,/水彩/);
 ...
 await at('video-c').locator('.canvas-node-body').click();await at('video-c').locator('.node-run').click();await page.waitForFunction(()=>JSON.parse(localStorage.getItem('zora.canvasNodes.v1'))[2].runState==='submitted');assert(await at('video-c').locator('.node-run').isDisabled());await at('video-c').getByRole('button',{name:'刷新生成结果'}).click();await page.waitForFunction(()=>JSON.parse(localStorage.getItem('zora.canvasNodes.v1'))[2].runState==='complete'); await page.screenshot({path:path.join(output,'workflow.png')});assert.equal(errors.length,0);await page.waitForTimeout(500);const downloads=await app.evaluate(()=>globalThis.downloadStates);assert(downloads.length>=1);assert(downloads.every(d=>d.state==='completed'));console.log(JSON.stringify({downloads}));console.log(JSON.stringify({passed:true,errors,mediaRequests}));
