...
try{
 ...
 await page.evaluate(async()=>{const {models}=await (await fetch('/api/models')).json();const nodes=[{id:'text-a',type:'text',label:'文字',x:70,y:90,prompt:'一只白猫'},{id:'image-b',type:'res-image',label:'图片',x:420,y:90,prompt:'水彩风格',modelId:models.find(m=>m.kind==='image').id},{id:'video-c',type:'res-video',label:'视频',x:770,y:90,prompt:'镜头缓慢推进',modelId:'grok-video-3',params:{videoMode:'i2v'}}];const list=JSON.parse(localStorage.getItem('zora.canvases.v1'));list[0].nodes=nodes;localStorage.setItem('zora.canvases.v1',JSON.stringify(list));localStorage.setItem('zora.canvasNodes.v1',JSON.stringify(nodes));});await page.reload();await canvas();
 ...
 await drag(at('video-c').locator('.port-left'),at('text-a').locator('.port-right'));assert.equal(await page.locator('.canvas-wires path').count(),3);
 await drag(at('image-b').locator('.port-right'),at('text-a').locator('.port-left'));assert.equal(await page.locator('.canvas-wires path').count(),3);
 const p=await at('text-a').locator('.port-right').boundingBox();await page.mouse.move(p.x+11,p.y+11);await page.mouse.down();await page.mouse.move(p.x+80,p.y+90);assert.equal(await page.locator('.wire-preview').count(),1);await page.keyboard.press('Escape');assert.equal(await page.locator('.wire-preview').count(),0);await page.mouse.up();
 await page.mouse.move(p.x+11,p.y+11);await page.mouse.down();await page.mouse.move(p.x+80,p.y+90);await page.mouse.up();assert.equal(await page.locator('.wire-preview').count(),0);assert.equal(await page.locator('.canvas-wires path').count(),3);
 await page.reload();await canvas();assert.equal(await page.locator('.canvas-wires path').count(),3);const saved=await readStore('zora.canvasNodes.v1');assert.deepEqual(saved[2].inputs,['image-b','text-a']);assert.deepEqual(saved[1].inputs,['text-a']);
 const boardRect=await page.locator('#canvas-board').boundingBox();await page.mouse.move(boardRect.x+400,boardRect.y+450);await page.mouse.wheel(0,200);await page.waitForTimeout(150);await drag(at('video-c').locator('.port-left'),at('text-a').locator('.port-right'));assert.equal(await page.locator('.canvas-wires path').count(),3);
 assert.equal(errors.length,0);await page.screenshot({path:path.join(output,'drag-wires.png')});console.log('PASS fan-in fan-out reverse drag duplicate cycle cancel reload zoom');
