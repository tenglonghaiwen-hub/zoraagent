...
try {
 ...
 await page.locator('#prompt-card').screenshot({path:path.join(output,'collapsed.png')});
 await page.locator('#prompt').fill('遮罩边界验证');
 await page.waitForTimeout(700);
 const paint=await page.locator('#prompt-card').evaluate(e=>{const c=getComputedStyle(e),p=getComputedStyle(e,'::before');return {overflow:c.overflow,backdrop:c.backdropFilter,clip:p.clipPath,paintBackdrop:p.backdropFilter,pointerEvents:p.pointerEvents};});
