...
try{
 ...
 await page.evaluate(()=>{const nodes=[{id:'top',type:'text',label:'文本',x:260,y:60},{id:'bottom',type:'res-image',label:'图片',x:260,y:260}];const list=JSON.parse(localStorage.getItem('zora.canvases.v1'));list[0].nodes=nodes;localStorage.setItem('zora.canvases.v1',JSON.stringify(list));localStorage.setItem('zora.canvasNodes.v1',JSON.stringify(nodes));});await page.reload();await canvas();await page.locator('[data-id="top"] .canvas-node-body').click();
 const box=await page.locator('[data-id="bottom"]').boundingBox();const point={x:box.x+100,y:box.y+40};assert(await page.evaluate(({x,y})=>document.elementFromPoint(x,y)?.tagName==='TEXTAREA',point));await page.mouse.click(point.x,point.y);await page.keyboard.type('Overlap input works');assert.equal(await page.locator('[data-id="top"] textarea').inputValue(),'Overlap input works');
 await page.locator('#theme-toggle').click();assert(await page.evaluate(({x,y})=>document.elementFromPoint(x,y)?.tagName==='TEXTAREA',point));await page.screenshot({path:path.join(output,'node-layer.png'),timeout:20000});assert.equal(errors.length,0);console.log('PASS overlapping text editor receives mouse and keyboard in both themes');
