...
try{
 ...
 await page.locator('#canvas-tool-toggle').click();const b=await page.locator('#canvas-board').boundingBox();await page.mouse.move(b.x+450,b.y+350);await page.mouse.wheel(0,200);await page.waitForTimeout(100);const zoom=await page.locator('#canvas-zoom-label').textContent();
 await page.locator('[data-tab="settings"]').first().click();await page.locator('#reduce-motion').check();await page.locator('#auto-download').check();
 await page.evaluate(()=>{const c=document.querySelector('#creation-kind');c.value='agent';c.dispatchEvent(new Event('change',{bubbles:true}));});await page.locator('#rail-toggle').click();
 await page.reload();assert(await page.locator('#reduce-motion').isChecked());assert(await page.locator('#auto-download').isChecked());assert.equal(await page.locator('#creation-kind').inputValue(),'agent');assert(await page.locator('#studio').evaluate(e=>e.classList.contains('rail-collapsed')));await canvas();assert.equal(await page.locator('#canvas-zoom-label').textContent(),zoom);assert.equal(await page.locator('#canvas-tool-toggle').getAttribute('data-tool'),'pan');assert.equal(errors.length,0);console.log('PASS settings mode rail zoom tool preserved across reload');
