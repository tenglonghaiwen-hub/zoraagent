...
try{
 ...
 for(const modelId of ['doubao-seedance-2.5','doubao-seedance-2-0-mini']){
  await page.evaluate(modelId=>window.__reloadCanvasNodes([{id:'doc-video',type:'res-video',label:'视频',modelId,x:100,y:30,prompt:'测试',params:{duration:-1}}]),modelId);
  await page.locator('.canvas-node[data-id="doc-video"] .canvas-node-body').click();
  const durations=await page.getByLabel('节点时长',{exact:true}).evaluate(el=>[...el.options].map(o=>({value:o.value,label:o.textContent})));
  assert(durations.some(o=>o.value==='-1'&&o.label==='自动'));assert(!durations.some(o=>Number(o.value)>15));
  assert.equal(await page.getByLabel('节点时长',{exact:true}).inputValue(),'-1');
  const resolutions=await page.getByLabel('节点分辨率',{exact:true}).evaluate(el=>[...el.options].map(o=>o.value));assert.deepEqual(resolutions,['480P','720P','1080P']);
 }
 assert.equal(errors.length,0);await writeFile(path.join(output,'results.json'),JSON.stringify({passed:true,errors},null,2));console.log('PASS Seedance 2.5 and Mini duration/resolution controls');
