...
try{
 await page.evaluate(()=>{localStorage.setItem('zora.auth.v1','1');location.hash='studio';localStorage.setItem('zora.workMode.v1','agent');localStorage.setItem('zora.session.v1',JSON.stringify({projects:[{id:'p',name:'测试'}],currentProjectId:'p',currentConversationId:'scroll-test',conversations:[{id:'scroll-test',projectId:'p',title:'位置测试',messages:Array.from({length:20},(_,i)=>({kind:'agent',text:'问题'+i,answer:'回答'.repeat(100),liveAgent:true,references:[],createdAt:Date.now()}))}]}));});
 await page.reload();await agent();
 const log=page.locator('.conversation-log');await log.waitFor({state:'visible'});
 await page.getByRole('button',{name:'打开对话 位置测试',exact:true}).click();
 assert(await log.evaluate(e=>Math.abs(e.scrollHeight-e.clientHeight-e.scrollTop)<3));
 await log.evaluate(e=>e.scrollTop=100);
 // Clicking the current conversation is also an explicit reopen.
 await page.getByRole('button',{name:'打开对话 位置测试',exact:true}).click();
 assert(await log.evaluate(e=>Math.abs(e.scrollHeight-e.clientHeight-e.scrollTop)<3));
 assert.equal(errors.length,0);console.log('PASS open and reopen locate latest message');
