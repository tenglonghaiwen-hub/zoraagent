...
try{
 ...
 await page.evaluate(()=>{const list=JSON.parse(localStorage.getItem('zora.canvases.v1'));list[0].nodes=[{id:'position-node',type:'res-image',label:'图片',modelId:'gpt-image-2',x:330,y:-270,prompt:'下拉定位验证'}];localStorage.setItem('zora.canvases.v1',JSON.stringify(list));localStorage.setItem('zora.canvasNodes.v1',JSON.stringify(list[0].nodes));});await page.reload();await canvas();
 // The image body is above the viewport, select through the board's normal node selection first.
 await page.evaluate(()=>{window.__reloadCanvasNodes([{id:'position-node',type:'res-image',label:'图片',modelId:'gpt-image-2',x:330,y:10,prompt:'下拉定位验证'}]);});
 await page.locator('.canvas-node[data-id="position-node"] .canvas-node-body').click();
 const records=[];
 for(const scale of [1,.75,1.25]){
  await page.evaluate(scale=>{document.querySelector('#canvas-nodes').style.transform='translate(90px,-260px) scale('+scale+')';},scale);
  const select=page.getByLabel('节点模型',{exact:true});await select.click();
  const rects=await select.evaluate(el=>({trigger:el.getBoundingClientRect().toJSON(),option:el.options[0].getBoundingClientRect().toJSON(),top:getComputedStyle(el,'::picker(select)').top,left:getComputedStyle(el,'::picker(select)').left}));records.push({scale,...rects});
  assert(Math.abs(rects.option.x-rects.trigger.x-7)<4,JSON.stringify(rects));assert(rects.option.y>=rects.trigger.bottom,JSON.stringify(rects));
  await page.screenshot({path:path.join(output,'position-'+scale+'.png')});await page.keyboard.press('Escape');
 }
 await writeFile(path.join(output,'results.json'),JSON.stringify({passed:true,records,errors},null,2));assert.equal(errors.length,0);console.log('PASS popup stays below trigger at 75%, 100% and 125% with canvas translation');
