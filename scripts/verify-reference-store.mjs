...
try{
 ...
 const id=await page.evaluate(async()=>{const {saveReference}=await import('/reference-store.js');const ref={file:new File(['test-image-bytes'],'original.png',{type:'image/png'})};await saveReference(ref);return ref.storageId;});
 await page.reload();
 ...
 assert(result.restored);assert.equal(result.name,'original.png');assert.equal(result.text,'test-image-bytes');assert(result.url.startsWith('blob:'));assert.equal(errors.length,0);
