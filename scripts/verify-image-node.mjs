...
try {
 ...
 await page.locator('[data-copy]').click();assert.equal(await page.locator('.canvas-image-node').count(),2);await page.reload();await canvas();assert.equal(await page.locator('.canvas-image-node').count(),2);await page.locator('.canvas-node-body').first().click();assert.equal(await page.getByRole('textbox',{name:'图片节点描述'}).inputValue(),'本地图片描述保存验证');await page.screenshot({path:path.join(output,'image-node.png')});console.log(JSON.stringify({passed:true,errors,blocked}));
