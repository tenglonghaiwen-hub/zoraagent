...
try {
 ...
 await page.evaluate(()=>{localStorage.setItem('zora.auth.v1','1');location.hash='studio';});await page.reload();await agent();
 await mainSend(Array.from({length:40},(_,i)=>'滚动边界验证文字 '+(i+1)).join('\n')); await page.locator('#prompt').fill(''); await page.locator('#prompt-card').screenshot({path:path.join(output,'collapsed.png')});
 await page.locator('#prompt').fill('遮罩边界验证');
 await page.waitForTimeout(700);
 console.log(await page.evaluate(()=>['#agent-workspace','.conversation-log','#prompt-card'].map(s=>({s,rect:document.querySelector(s).getBoundingClientRect().toJSON(),gap:getComputedStyle(document.querySelector(s)).gap})))); assert(await page.evaluate(()=>Math.abs(document.querySelector('.conversation-log').getBoundingClientRect().bottom-document.querySelector('#prompt-card').getBoundingClientRect().top)<1)); await page.locator('.conversation-log').evaluate(e=>e.scrollTop=0); const paint=await page.locator('#prompt-card').evaluate(e=>{const c=getComputedStyle(e),p=getComputedStyle(e,'::before');return {overflow:c.overflow,backdrop:c.backdropFilter,clip:p.clipPath,paintBackdrop:p.backdropFilter,pointerEvents:p.pointerEvents};});
 assert.equal(paint.overflow,'visible');assert.equal(paint.backdrop,'none');assert.match(paint.clip,/inset/);assert.equal(paint.pointerEvents,'none');
 await page.screenshot({path:path.join(output,'expanded.png')});
 await page.getByRole('button',{name:'创作类型',exact:true}).click();
 assert(await page.getByRole('option',{name:'Agent 模式',exact:true}).isVisible());
 await page.screenshot({path:path.join(output,'menu.png')});
 await writeFile(path.join(output,'result.json'),JSON.stringify({paint,errors,blocked,menuVisible:true},null,2));
 console.log(JSON.stringify({paint,errors,blocked,output}));
