...
try{
 const result=await page.evaluate(async()=>{
  const c=document.createElement('canvas');c.width=1500;c.height=1500;const ctx=c.getContext('2d');const pixels=ctx.createImageData(1500,1500);for(let i=0;i<pixels.data.length;i++)pixels.data[i]=Math.random()*256;ctx.putImageData(pixels,0,0);
  const original=c.toDataURL('image/png');const ref={contentUrl:original,type:'image/png'};const {prepareMediaReference}=await import('/media-reference.js?v=studio147');const packed=await prepareMediaReference(ref,()=>{});return {before:original.length,after:packed.contentUrl.length,unchanged:ref.contentUrl===original,type:packed.type};
 });assert(result.before>3000000);assert(result.after<=3000000);assert(result.unchanged);assert.equal(result.type,'image/png');console.log(JSON.stringify(result));
