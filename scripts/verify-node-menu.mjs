...
try{await page.waitForLoadState('domcontentloaded');await page.evaluate(()=>{localStorage.setItem('zora.auth.v1','1');location.hash='studio';});await page.reload();await seed();const b=await page.locator('#canvas-board').boundingBox();const count=await page.locator('.canvas-node').count();await page.mouse.dblclick(b.x+500,b.y+250);assert.equal(await page.locator('.canvas-node').count(),count);assert(await page.locator('#canvas-context-menu').isVisible());assert.equal(await page.locator('#canvas-context-menu [data-canvas-node]').count(),3);await page.screenshot({path:path.join(output,'node-menu.png')});await page.locator('#canvas-context-menu [data-canvas-node="res-video"]').click();assert.equal(await page.locator('.canvas-node').count(),count+1);assert(await page.locator('.canvas-video-node').isVisible());assert(!await page.locator('#canvas-context-menu').isVisible());console.log(JSON.stringify({passed:true,errors,blocked}));}finally{await app.close();await fixture.close();}