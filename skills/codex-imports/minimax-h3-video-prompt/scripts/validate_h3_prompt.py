...
def main() -> int:
    ...
    if ratio not in RATIOS:
        errors.append(
            "画幅必须是 adaptive、21:9、16:9、4:3、1:1、3:4 或 9:16"
        )

    if args.mode == "text-to-video":
        if args.images + args.videos + args.audios or args.first_frame or args.last_frame:
    ...
    if args.mode in ("reference-to-video", "video-editing"):
        if args.first_frame or args.last_frame:
            errors.append("多模态参考模式不能与首帧或尾帧输入混用")
        if args.images > 9:
