...
export function validateNetwork(value){
 if(!value||!['system','environment','direct','proxy'].includes(value.mode))throw Error('网络模式无效');
 let proxy='';
 if(value.mode==='proxy'){
...
export function readNetwork(directory){const file=path.join(directory,'network.json');return fs.existsSync(file)?validateNetwork(JSON.parse(fs.readFileSync(file,'utf8').replace(/^\uFEFF/,''))):{mode:'system',proxy:''};}
