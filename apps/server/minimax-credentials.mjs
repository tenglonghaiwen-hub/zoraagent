...
export function loadStoredMinimaxKey(root,{env=process.env,platform=process.platform,exists=fs.existsSync,exec=execFileSync}={}){
 if(env.MINIMAX_API_KEY)return 'environment';
 const file=path.join(root,'runtime','minimax-key.dpapi');
 if(!exists(file))return 'fallback';
 if(platform!=='win32')throw Error('MiniMax 官方 加密凭据需要 Windows 当前用户解密');
 try{
  ...
  if(!key)throw Error();env.MINIMAX_API_KEY=key;return 'encrypted-store';
 }catch{throw Error('无法解密MiniMax 官方 已保存凭据，请使用原 Windows 用户启动或重新配置凭据');}
}
