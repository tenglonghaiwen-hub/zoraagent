import {conversationDay,conversationViewport} from './conversation-view.js?v=studio206';
import {createMessageReadState,messageBadgeText} from './message-unread.js?v=studio205';
import {selectConversationReferences} from './conversation-references.js?v=studio201';
import {refreshImageSuite,imageSuitePageStatus} from './image-suite.js?v=studio200';
import {bindExecutionDisclosure} from './execution-disclosure.js';
import {attachGenerationReceipts,applyGenerationReceipt,verifyGenerationRetry} from './agent-generation-tasks.js?v=studio200';
...
import {initLoginPage, initUserMenu, updateUserBalance, clearUserMenu, initMembershipPanel, updateUserVipUI} from './login-handler.js?v=studio203';
...
(function initWorkModeCanvas(){
  ...
  function renderNodes(){
    ...
    for(const n of nodes){
      ...
      if(['res-image','t2i','i2i','res-video','t2v','i2v'].includes(n.type)){
        ...
        const isMediaResult=['res-image','res-video'].includes(n.type);
        el.classList.add('canvas-image-node');
        if(videoNode)el.classList.add('canvas-video-node');
        if(isMediaResult)el.classList.add('canvas-media-result-node');
        const body=el.querySelector('.canvas-node-body');
        body.replaceChildren();
        if(videoNode&&n.mediaData&&!n.outputUrl){
          const media=document.createElement(n.mediaType?.startsWith('video/')?'video':n.mediaType?.startsWith('audio/')?'audio':'img');
          media.src=n.mediaData;
        ...
        else if(n.outputUrl||n.imageData){
          const isVid = n.outputUrl && videoNode;
          const media=document.createElement(isVid ? 'video' : 'img');
          media.src=n.outputUrl||n.imageData;
          media.alt=n.label||'节点素材';
          media.draggable=false;
        ...
        if(isMediaResult){
          el.addEventListener('dblclick',(e)=>{
            if(e.target.closest('button, input, textarea, select, .canvas-node-port, a')) return;
            const targetUrl = n.outputUrl || n.imageData || n.mediaData;
            if(targetUrl){
              e.stopPropagation();
              openGeneratedPreview(targetUrl, videoNode ? 'video' : 'image');
            }
          });
        }
        ...
        if(selected.has(n.id)){
          ...
          if(toolbar.querySelector('[data-preview]')){
            toolbar.querySelector('[data-preview]').onclick=(e)=>{
              e.stopPropagation();
              openGeneratedPreview(n.outputUrl||n.imageData||n.mediaData, videoNode?'video':'image');
            };
          }
          if(toolbar.querySelector('[data-download]')){
            toolbar.querySelector('[data-download]').onclick=(e)=>{
              e.stopPropagation();
      ...
      el.addEventListener('pointerdown',(e)=>{
        ...
        if(e.target.closest('button, input, textarea, select, .canvas-node-port, a, .image-node-editor, .image-node-toolbar'))return;
        e.stopPropagation();
        ...
        if(!e.shiftKey){
          if(!selected.has(n.id)){
            selected.clear();
            selected.add(n.id);
            for(const nodeDom of nodesEl.querySelectorAll('.canvas-node')){
              nodeDom.classList.toggle('is-selected', nodeDom.dataset.id===n.id);
            }
          }
        }else{
          if(selected.has(n.id)){
        ...
        for(const m of nodes){
          if(m.id===n.id || selected.has(m.id)){
            startPosMap.set(m.id, {x:m.x||0, y:m.y||0});
          }
        }
      ...
      el.addEventListener('pointermove',(e)=>{
        if(!dragging)return;
        const rawDx=e.clientX-sx, rawDy=e.clientY-sy;
        if(Math.hypot(rawDx,rawDy)>3) moved=true;
        if(!moved)return;
        ...
        for(const m of nodes){
          const start=startPosMap.get(m.id);
          if(!start)continue;
          m.x=Math.round(start.x+dx);
          m.y=Math.round(start.y+dy);
          const nodeDom=nodesEl.querySelector(`.canvas-node[data-id="${m.id}"]`);
          if(nodeDom){
            nodeDom.style.left=m.x+'px';
            nodeDom.style.top=m.y+'px';
          }
        }
        drawWires();
      });
  ...
  window.__canvasAddNode=addNode;
  window.__canvasInsertMediaNode=function({ url, kind='image', prompt='', at=null, batchIndex=0, switchView=true } = {}){
    if(!url) return null;
    if(switchView && mode !== 'canvas'){
      setWorkMode('canvas');
      window.__canvasEnsureProject?.({forceNew:false});
    }
    ...
    if(!targetPos){
      ...
      targetPos = {
        x: Math.round(cx - 100 + (batchIndex * 260) % 780 + (Math.random() * 20 - 10)),
        y: Math.round(cy - 60 + Math.floor((batchIndex * 260) / 780) * 180 + (Math.random() * 20 - 10))
      };
    }
    ...
    const node = addNode(type, targetPos, {
      label,
      note,
      prompt: cleanPrompt,
      outputUrl: url,
  ...
  // Canvas board Drag & Drop for agent media results and files
  board.addEventListener('dragover', (e)=>{
    const types = Array.from(e.dataTransfer?.types || []);
    if (types.includes('application/x-zora-gen') || types.includes('Files') || types.includes('text/uri-list') || types.includes('text/plain')) {
      e.preventDefault();
  ...
  board.addEventListener('drop', async (e)=>{
    ...
    e.stopPropagation();
    board.classList.remove('is-drop-target');
    const dropPos = toContent(e);

    // 1. application/x-zora-gen from Agent task result
    const raw = e.dataTransfer.getData('application/x-zora-gen');
    if (raw) {
      try {
        const data = JSON.parse(raw);
        if (data && data.url) {
          window.__canvasInsertMediaNode({
            url: data.url,
    ...
    // 3. Local Files dropped directly onto board
    ...
    if (files.length) {
      files.forEach((file, idx) => {
        ...
        const reader = new FileReader();
        reader.onload = () => {
          const at = { x: dropPos.x + idx * 24, y: dropPos.y + idx * 24 };
          addNode(isVideo ? 'res-video' : 'res-image', at, {
            note: file.name,
            outputUrl: '',
            imageData: isVideo ? '' : reader.result,
            mediaData: isVideo ? reader.result : '',
            mediaType: file.type
          });
        };
        reader.readAsDataURL(file);
      });
      toast(`已放置 ${files.length} 个媒体节点`);
    }
  });
...
function initGatewaySettings() {
  const modePills = document.querySelectorAll('.gateway-mode-pills [data-gateway-mode]');
  const cloudBox = document.getElementById('gateway-cloud-config');
  const cloudInput = document.getElementById('gateway-cloud-url');
  const testBtn = document.getElementById('gateway-test-btn');
  const adminLink = document.getElementById('gateway-admin-link');
  const statusText = document.getElementById('gateway-status-text');

  if (!modePills.length) return;

  const cfg = getGatewayConfig();
  if (cloudInput) {
    if (!cloudInput.value || isLegacyGatewayUrl(cloudInput.value)) {
      cloudInput.value = DEFAULT_CLOUD_GATEWAY;
      setGatewayConfig({ cloudUrl: DEFAULT_CLOUD_GATEWAY });
    } else {
      cloudInput.value = cfg.cloudUrl || DEFAULT_CLOUD_GATEWAY;
    }
  }

  function renderMode(mode) {
    modePills.forEach(btn => {
      const active = btn.dataset.gatewayMode === mode;
      btn.classList.toggle('active', active);
      btn.setAttribute('aria-pressed', String(active));
    });
    if (cloudBox) cloudBox.hidden = mode !== 'cloud';
    if (adminLink) {
      adminLink.hidden = mode !== 'cloud';
      const curCloud = (cloudInput?.value || cfg.cloudUrl || DEFAULT_CLOUD_GATEWAY).replace(/\/+$/, '');
      adminLink.href = curCloud ? `${curCloud}/admin` : '#';
    }
  }
  ...
  modePills.forEach(btn => {
    btn.addEventListener('click', () => {
      const nextMode = btn.dataset.gatewayMode;
      let curCloud = (cloudInput?.value || DEFAULT_CLOUD_GATEWAY).trim();
      if (isLegacyGatewayUrl(curCloud)) {
        curCloud = DEFAULT_CLOUD_GATEWAY;
        if (cloudInput) cloudInput.value = DEFAULT_CLOUD_GATEWAY;
      }
      setGatewayConfig({ mode: nextMode, cloudUrl: curCloud });
      renderMode(nextMode);
      toast(nextMode === 'cloud' ? '已连接 Cloudflare 全球生产网关' : '已切换为本地开发服务');
      if (statusText) {
  ...
  cloudInput?.addEventListener('change', () => {
    let val = cloudInput.value.trim();
    if (isLegacyGatewayUrl(val)) {
      val = DEFAULT_CLOUD_GATEWAY;
      cloudInput.value = val;
    }
    if (val) {
      setGatewayConfig({ cloudUrl: val });
      if (adminLink) adminLink.href = `${val.replace(/\/+$/, '')}/admin`;
      toast('云网关地址已保存');
      refreshModelCatalog();
    }
  });
  ...
  testBtn?.addEventListener('click', async () => {
    ...
    const currentCfg = getGatewayConfig();
    let targetUrl = currentCfg.mode === 'cloud' ? (cloudInput?.value || currentCfg.cloudUrl) : currentCfg.localUrl;
    if (currentCfg.mode === 'cloud' && isLegacyGatewayUrl(targetUrl)) {
      targetUrl = DEFAULT_CLOUD_GATEWAY;
      if (cloudInput) cloudInput.value = DEFAULT_CLOUD_GATEWAY;
      setGatewayConfig({ cloudUrl: DEFAULT_CLOUD_GATEWAY });
    }
    statusText.className = 'gateway-status-text is-testing';
    statusText.textContent = '正在探测网关连通性…';
    testBtn.disabled = true;

    try {
      const res = await testGatewayConnection(targetUrl);
      if (res.ok) {
        statusText.className = 'gateway-status-text is-success';
        statusText.textContent = `● 连接正常 (${res.latency}ms) · 发现 ${res.modelCount} 个可用模型`;
        toast('网关连通测试成功');
      } else {
        statusText.className = 'gateway-status-text is-error';
        let friendlyError = res.error || '未响应';
        if (/Failed to fetch/i.test(friendlyError)) {
          if (/zora-api\.workers\.dev/i.test(targetUrl || '')) {
            friendlyError = '默认示例域名未部署。本地测试请执行 npx wrangler dev 并填入 http://127.0.0.1:8787，或填入你部署的真实域名';
          } else if (/127\.0\.0\.1:8787|localhost:8787/i.test(targetUrl || '')) {
            friendlyError = '本地 Worker 未启动，请在 apps/cloudflare-worker 目录执行 npx wrangler dev';
          } else {
            friendlyError = '网络请求失败，请检查域名解析、CORS 或服务器状态';
          }
        }
        statusText.textContent = `× 连接异常 (${res.latency}ms): ${friendlyError}`;
        toast('网关连接失败，请核对地址或服务状态');
      }
    } catch (e) {
      statusText.className = 'gateway-status-text is-error';
      statusText.textContent = `× 探测错误: ${e.message}`;
    } finally {
...
// Non-modal message drawer and account popover with remote cloud gateway notifications
const messagePanel=$('#message-panel'), accountMenu=$('#account-menu');let messageKind='activity';let panelReturn=null;
let remoteMessages = [];
const messageButton = $('#messages-open');
const messageBadge = document.createElement('span');
messageBadge.id='messages-unread-badge';messageBadge.className='message-unread-badge';
messageBadge.hidden=true;messageBadge.setAttribute('aria-hidden','true');
const messageIcon=messageButton.querySelector('.tb-icon');
const messageIconAnchor=document.createElement('span');messageIconAnchor.className='message-icon-anchor';
messageIcon.before(messageIconAnchor);messageIconAnchor.append(messageIcon,messageBadge);
const messageOwner=()=>`${getGatewayConfig().effectiveBase||'local'}:${getUser()?.id||'guest'}`;
let inboxOwner=messageOwner(), inboxRead=createMessageReadState(localStorage,'zora.messages.read.v1:'+inboxOwner);
let messageSync=null;
function ensureMessageOwner(){
 const owner=messageOwner();
 if(owner!==inboxOwner){inboxOwner=owner;remoteMessages=[];inboxRead=createMessageReadState(localStorage,'zora.messages.read.v1:'+owner);if(!messagePanel.hidden)renderMessages();}
 return owner;
}
function updateMessageBadge(){
 const count=inboxRead.count(remoteMessages);
 messageBadge.textContent=messageBadgeText(count);messageBadge.hidden=count===0;
 messageBadge.classList.toggle('is-dot',count===1);
 messageButton.classList.toggle('has-unread',count>0);
 messageButton.setAttribute('aria-label',count?`消息，${count} 条未读`:'消息，无未读消息');
 messageButton.title=count?`${count} 条未读消息`:'消息';
}
function markVisibleMessagesRead(){
 if(!messagePanel.hidden&&messageKind==='official'&&!document.hidden){inboxRead.markRead(remoteMessages);updateMessageBadge();}
}
async function syncRemoteMessages() {
 const owner=ensureMessageOwner();updateMessageBadge();
 if(messageSync?.owner===owner)return messageSync.promise;
 const promise=(async()=>{
  try {
    const data=await fetchMessages();
    if(owner!==messageOwner())return;
    if(data?.ok&&Array.isArray(data.messages)){
      remoteMessages=data.messages;updateMessageBadge();
      if(!messagePanel.hidden){renderMessages();markVisibleMessagesRead();}
    }
  } catch {}
 })();
 messageSync={owner,promise};
 try {await promise;}finally{if(messageSync?.promise===promise)messageSync=null;}
}
syncRemoteMessages();
setInterval(()=>{if(!document.hidden)syncRemoteMessages();},30000);
window.addEventListener('focus',()=>syncRemoteMessages());
window.addEventListener('zora:user-refreshed',()=>syncRemoteMessages());
window.addEventListener('hashchange',()=>syncRemoteMessages());
document.addEventListener('visibilitychange',()=>{if(!document.hidden)syncRemoteMessages();});
window.addEventListener('storage',event=>{
 if(event.key==='zora.messages.read.v1:'+inboxOwner){inboxRead=createMessageReadState(localStorage,event.key);updateMessageBadge();}
});
...
function renderMessages(){
  ...
  if (messageKind === 'official') {
    if (!remoteMessages.length) {
      const empty=document.createElement('div');
      empty.className='message-empty';
      empty.textContent='暂无官方消息或系统公告。';
      feed.append(empty);
      return;
    }
    for (const msg of remoteMessages) {
      ...
      icon.textContent=msg.kind==='official'?'📢':'✦';
      ...
      const meta=document.createElement('small');
      const dateStr = msg.createdAt ? new Date(msg.createdAt).toLocaleDateString('zh-CN') : '最新';
      meta.textContent=`${msg.userId === '*' ? '全员公告' : '专属私信'} · ${dateStr}`;
      content.append(title,p,meta);
  ...
  if (!drafts.length){
    const empty=document.createElement('div');
    empty.className='message-empty';
    empty.textContent='暂无任务消息。创建任务草稿后可在这里查看本地记录。';
    feed.append(empty);
    return;
  }
  for(const draft of [...drafts].reverse()){
    ...
    const content=document.createElement('div');
    const title=document.createElement('h3');
    title.textContent='任务草稿已整理';
    const p=document.createElement('p');
    p.textContent=draft.prompt;
    const meta=document.createElement('small');
    meta.textContent=`${draft.count} 条 · ${draft.paused?'已暂停草稿':'未提交生成'} · 本地创作`;
    content.append(title,p,meta);
    item.append(icon,content);
    feed.append(item);
  }
}
$('#messages-open').onclick=()=>{
  const open=messagePanel.hidden;
  closeAccount();
  if(open){
    ensureMessageOwner();
    messageKind='official';
    document.querySelectorAll('[data-message-kind]').forEach(button=>button.classList.toggle('active',button.dataset.messageKind==='official'));
    messagePanel.hidden=false;
    renderMessages();markVisibleMessagesRead();syncRemoteMessages();
    $('#messages-open').setAttribute('aria-expanded','true');
    panelReturn=$('#messages-open');
    $('#messages-close').focus();
  } else closeMessages();
};
$('#messages-close').onclick=()=>{closeMessages();$('#messages-open').focus();};
document.querySelectorAll('[data-message-kind]').forEach(b=>b.onclick=()=>{messageKind=b.dataset.messageKind;document.querySelectorAll('[data-message-kind]').forEach(x=>x.classList.toggle('active',x===b));renderMessages();markVisibleMessagesRead();});
...
function formatMsgTime(ts){if(!ts)return '';const d=new Date(ts);if(Number.isNaN(d.getTime()))return '';const weeks=['星期日','星期一','星期二','星期三','星期四','星期五','星期六'];const hh=String(d.getHours()).padStart(2,'0');const mm=String(d.getMinutes()).padStart(2,'0');return weeks[d.getDay()]+hh+':'+mm;}
...
function renderConversation(){const open=!!currentConversation;$('#create').classList.toggle('conversation-active',open);conversationLog.hidden=!open;conversationLog.replaceChildren();renderConversationRail();if(!open)return;let lastDay;for(const [messageIndex,message] of currentConversation.messages.entries()){const day=conversationDay(message.createdAt);if(day!==lastDay){conversationLog.append(chatNode('h2','conversation-date',day));lastDay=day;}const viewKey=currentConversation.id+':'+(message.id||messageIndex);if(message.kind && message.kind!=='agent'){renderMediaTask(message);if(conversationLog.lastElementChild)conversationLog.lastElementChild.dataset.chatKey=viewKey+':media';continue;}const user=chatNode('article','conversation-user');user.dataset.chatKey=viewKey+':user';if(message.references&&message.references.length){const refs=chatNode('div','conversation-references');for(const a of message.references){const mime=(a.file&&a.file.type)||'';const media=document.createElement(mime.startsWith('video/')||a.kind==='video'?'video':'img');if(!a.url){refs.append(chatNode('span','reference-unavailable','原素材不可用，请重新添加'));continue;}media.src=a.url;media.onerror=()=>media.replaceWith(chatNode('span','reference-unavailable','素材加载失败，请重新添加'));media.setAttribute('aria-label',(a.file&&a.file.name)||a.reference||'参考');if(media.tagName==='IMG')media.alt=(a.file&&a.file.name)||'参考';else media.controls=true;refs.append(media);}user.append(refs);}user.append(chatNode('p','conversation-bubble',message.text),buildUserMsgBar(message));conversationLog.append(user);const reply=chatNode('article','conversation-reply');reply.dataset.chatKey=viewKey+':reply';reply.append(chatNode('strong','conversation-agent','Zora Agent · 界面演示'));reply.append(chatNode('p','','已记录你的创作需求。以下为根据所选参数整理的本地任务草稿，尚未经过 Agent 规划或提交生成。'));const card=chatNode('section','conversation-task-card');card.append(chatNode('h3','',`任务清单（${message.count}）`));for(let i=0;i<message.count;i++){const row=chatNode('label','conversation-task');const check=document.createElement('input');check.type='checkbox';check.checked=true;check.disabled=message.cancelled;check.setAttribute('aria-label',`选择任务 ${i+1}`);row.append(check,chatNode('span','',`${i+1}. ${message.text}`));card.append(row);}card.append(chatNode('p','conversation-meta',message.meta));const actions=chatNode('div','conversation-actions');const cancel=chatNode('button','secondary',message.cancelled?'已取消草稿':'取消草稿');cancel.disabled=!!message.cancelled;cancel.onclick=()=>{message.cancelled=true;renderConversation();};const generate=chatNode('button','','生成服务待接入');generate.disabled=true;actions.append(chatNode('span','','积分待报价'),cancel,generate);card.append(actions);reply.append(card);const suggestions=chatNode('div','conversation-suggestions');for(const text of ['补充分镜细节','调整画面风格','继续添加参考素材']){const b=chatNode('button','',text);b.onclick=()=>{if(text==='继续添加参考素材')$('#files').click();else{$('#prompt').value=text;$('#prompt').focus();}};suggestions.append(b);}reply.append(suggestions,chatNode('small','conversation-meta','本地界面演示 · 消息已保存 · 素材保留 7 天'));if(!message.liveAgent){if(!message.answer)message.answer=message.text||'';reply.append(buildAgentMsgBar(message));}conversationLog.append(reply);}}
...
function renderMediaTask(message,target=conversationLog){
 ...
 const heading=chatNode('div','media-task-heading');const prompt=chatNode('p','media-task-prompt',message.text);heading.append(prompt);if((message.text||'').length>220){prompt.classList.add('is-collapsed');const expand=chatNode('button','media-prompt-expand','展开提示词');expand.type='button';expand.setAttribute('aria-expanded','false');expand.onclick=()=>{const collapsed=prompt.classList.toggle('is-collapsed');expand.textContent=collapsed?'展开提示词':'收起提示词';expand.setAttribute('aria-expanded',String(!collapsed));};heading.append(expand);}heading.append(chatNode('span','conversation-meta',message.meta));const details=document.createElement('details');details.className='media-task-details';details.append(chatNode('summary','','详细信息'),chatNode('p','',`${message.kind==='video'?'视频':'图片'}任务 · ${message.count} 个结果 · ${message.meta} · ${message.genError||message.genStatus||'待发送'}`));{let requestLoading=false,requestLoaded=false;const loadRequest=async()=>{details.open=true;if(requestLoading||requestLoaded)return;requestLoading=true;try{if(message.imageSuite){await refreshImageSuite(message,authFetch);let pre=details.querySelector('.request-body');if(!pre){pre=chatNode('pre','request-body');pre.style.cssText='white-space:pre-wrap;overflow-wrap:anywhere;max-height:400px;overflow:auto';details.append(pre);}pre.textContent=JSON.stringify({sharedStyle:message.imageSuite.sharedStyle,items:message.imageSuite.items.map(item=>({title:item.title,prompt:item.prompt,taskId:item.task?.id,status:imageSuitePageStatus(item),requests:item.task?.requests||[],error:item.queryError||item.task?.pollError}))},null,2);requestLoaded=true;saveSession();return;}if(!message.genBatchId){let pre=details.querySelector('.request-body');if(!pre){pre=chatNode('pre','request-body');details.append(pre);}pre.textContent='此记录没有本地生成任务编号，无法查询实际提交请求体。';return;}const r=await authFetch('/api/generation-tasks/'+encodeURIComponent(message.genBatchId));const j=await r.json();if(!r.ok)throw Error(j.error||'读取失败');let pre=details.querySelector('.request-body');if(!pre){pre=chatNode('pre','request-body');pre.style.cssText='white-space:pre-wrap;overflow-wrap:anywhere;max-height:400px;overflow:auto';details.append(pre);}pre.textContent=j.task?.requests?.length?JSON.stringify(j.task.requests,null,2):'此任务未保存提交请求体；旧任务不能追溯还原。';requestLoaded=true;}catch(e){let note=details.querySelector('.request-error');if(!note){note=chatNode('p','request-error');details.append(note);}note.textContent=e.message;}finally{requestLoading=false;}};details.addEventListener('mouseenter',loadRequest);details.addEventListener('focusin',loadRequest);details.addEventListener('mouseleave',()=>{if(!details.contains(document.activeElement))details.open=false;});details.addEventListener('toggle',()=>{if(details.open)void loadRequest();});}heading.append(details);header.append(refs,heading);article.append(header);
 const results=chatNode('div','media-task-results');
 for(let i=0;i<message.count;i++){const resultUrl=message.genUrls?.[i]||(i===0?message.genUrl:null);const frame=chatNode('div','media-task-frame');const [w,h]=(message.ratio||'16:9').split(':').map(Number);frame.style.aspectRatio=`${w} / ${h}`;frame.style.width=`min(100%, ${Math.min(280,220*w/h)}px)`;const page=message.imageSuite?.items[i];const st=message.cancelled?'已取消':page?`${i+1}. ${page.title} · ${imageSuitePageStatus(page)}`:(message.genStatus||'待生成');frame.append(chatNode('span','media-task-status',st));if(message.genUrls?.[i]||(i===0&&message.genUrl)){const media=Object.assign(document.createElement(message.kind==='video'?'video':'img'),{src:message.genUrls?.[i]||message.genUrl,controls:false,className:'media-task-result',draggable:true});if(media.tagName==='IMG')media.alt=page?.title||'生成结果';media.title='点击放大预览，拖到画板可放置为节点';media.tabIndex=0;media.setAttribute('role','button');media.setAttribute('aria-label','打开生成素材预览');media.style.cursor='zoom-in';media.addEventListener('dragstart',e=>{try{e.dataTransfer.setData('application/x-zora-gen',JSON.stringify({url:resultUrl,kind:message.kind||'image',prompt:message.text||''}));e.dataTransfer.setData('text/uri-list',resultUrl);e.dataTransfer.setData('text/plain',resultUrl);e.dataTransfer.effectAllowed='copy';}catch{}});media.addEventListener('click',()=>openGeneratedPreview(resultUrl,message.kind||'image'));media.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();openGeneratedPreview(resultUrl,message.kind||'image');}});const insertQuick=chatNode('button','frame-insert-btn','⊞ 画板');insertQuick.type='button';insertQuick.title='将此项插入为画板节点';insertQuick.onclick=e=>{e.stopPropagation();if(typeof window.__canvasInsertMediaNode==='function'){window.__canvasInsertMediaNode({url:resultUrl,kind:message.kind||'image',prompt:message.text||'',batchIndex:i});}};frame.append(media,insertQuick);}else{frame.append(chatNode('span','media-task-placeholder',`${message.kind==='video'?'视频':'图片'}预览${message.count>1?' '+(i+1):''}`));}results.append(frame);}article.append(results);
 const actions=chatNode('div','media-task-actions');
 const validResultUrls=[...(message.genUrls||[]),message.genUrl].filter(Boolean);
 const uniqueResultUrls=[...new Set(validResultUrls)];
 if(uniqueResultUrls.length){
  const toCanvas=chatNode('button','btn-insert-canvas',uniqueResultUrls.length>1?'⊞ 全部插入画板':'⊞ 插入画板');
  toCanvas.title='将生成结果作为素材节点插入当前画布';
  toCanvas.onclick=()=>{if(typeof window.__canvasInsertMediaNode==='function'){uniqueResultUrls.forEach((url,idx)=>{window.__canvasInsertMediaNode({url,kind:message.kind||'image',prompt:message.text||'',batchIndex:idx});});}};
  const dl=chatNode('button','','下载');
  dl.onclick=async()=>{for(const url of (message.imageSuite?uniqueResultUrls:uniqueResultUrls.slice(0,1)))await downloadGenerated(url,message.kind||'image');};
  const useRef=chatNode('button','','加入参考');
  useRef.onclick=async()=>{for(const url of (message.imageSuite?uniqueResultUrls:uniqueResultUrls.slice(0,1)))await useGeneratedAsReference(url,message.kind||'image');};
  actions.append(toCanvas,dl,useRef);
 }
 const edit=chatNode('button','','重新编辑');edit.onclick=()=>{if(message.imageSuite){$('#creation-kind').value='agent';kindChanged();$('#prompt').value='请按以下整套图片方案调整并先展示方案，未经确认不要提交生成。\n'+message.text;$('#prompt').focus();toast('已恢复整套方案，请核对输入框中的参考素材');return;}$('#creation-kind').value=message.kind;kindChanged();$('#model').value=message.modelId;modelChanged();for(const id of ['ratio','resolution','duration','concurrency','video-mode']){const el=$('#'+id);if(!el)continue;const key=id==='video-mode'?'videoMode':id;if(message[key]!=null)el.value=message[key];}$('#count').value=message.count;$('#prompt').value=message.text;syncPickers();$('#prompt').focus();toast('已恢复文字和参数；参考素材请在输入栏核对');};
 const retry=chatNode('button','',message.imageSuite?'重新规划整套':'再次生成');retry.disabled=!!message.cancelled||message.genUnknown||message.genPending||message.genStatus==='生成中';retry.onclick=async()=>{if(message.imageSuite){edit.onclick();return;}retry.disabled=true;const allowed=await verifyGenerationRetry(message);saveSession();redraw();if(!allowed)return;message.genUnknown=false;message.referenceError=message.genError||message.referenceError;message.genBatchId=null;message.genPending=false;message.genRevision=null;message.genTaskIds=[];message.genTaskId=null;message.genUrls=[];message.genUrl=null;message.genStatus='生成中';message.genError='';redraw();try{await submitMediaGeneration(message);}catch(e){message.genError=e.submissionUnknown?'':e.message||'生成失败';message.genPollError=e.submissionUnknown?e.message:'';message.genStatus=e.submissionUnknown?'结果待确认':'生成失败';}redraw();};
...
async function prepareAgentReferences(references=[]){
 ...
 return Promise.all(references.map(async reference=>{
  if(reference.storageId&&!(reference.file instanceof Blob)){const restored=await restoreReference(reference);if(!restored&&!durableMediaUrl(reference.url))throw Error('本机未找到已保存的原素材：'+(reference.reference||reference.file?.name||'素材')+'。请核对当前客户端及会话。');}await saveReference(reference);const item=await prepareMediaReference(reference,fileToDataUrl);
...
async function submitMediaGeneration(message){
 if(message.imageSuite){
  await refreshImageSuite(message,authFetch);saveSession();
  for(const url of message.genUrls.filter(Boolean)){try{addGeneratedAsset({kind:'image',url,meta:message.modelId});}catch{}await autoDownloadGenerated({...message,genTaskId:null,genUrl:url});}
  return {suite:message.imageSuite};
 }
...
card.append(add,apply,openTasks,gen);reply.append(card);}reply.append(chatNode('small','conversation-meta',m.pending?'请等待本次回复完成':m.recovering?'正在核对后台任务状态，请勿重复发送':m.error?'本次请求未完成，可修改后重试':(m.added?'任务已同步到清单 · 可应用到创作栏或直接生成':'可继续对话 · Agent 可预览任务并提交生成')));if(!m.pending&&!m.recovering){reply.append(buildAgentMsgBar(m));}});};
...
$('#send-prompt').onclick=async()=>{
 ...
 if(!currentConversation){currentConversation={id:'c-'+Date.now()+'-'+Math.random().toString(36).slice(2,7),title:text.slice(0,24),messages:[],projectId:currentProjectId,createdAt:Date.now(),updatedAt:Date.now()};conversations.unshift(currentConversation);}else{touchConversation(currentConversation);ensureConversationId(currentConversation);}const conversation=currentConversation;const message={id:crypto.randomUUID(),text,kind:'agent',count:0,meta:'',references:[],referenceContextCleared:conversation.inheritReferences===false,liveAgent:true,pending:true,createdAt:Date.now()};try{const selected=selectConversationReferences({text,explicit:mentionedAssets(),messages:conversation.messages,inherit:conversation.inheritReferences!==false});message.references=selected.references;message.referencesInherited=selected.inherited;message.referenceContextCleared=!message.references.length&&conversation.inheritReferences===false;}catch(error){toast(error.message);return;}conversation.messages.push(message);openAtLatest=true;$('#prompt').value='';agentSending=true;$('#send-prompt').disabled=true;saveSession();renderConversation();
 try{await Promise.all(message.references.map(r=>saveReference(r)));saveSession();const refs=await prepareAgentReferences(message.references);const response=await authFetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messageId:message.id,conversationId:conversation.backendId,history:conversation.messages.filter(m=>m!==message).slice(-10).flatMap(m=>[{role:'user',message:m.text||''},...(m.answer?[{role:'assistant',reply:m.answer}]:[])]),modelId:$('#model').value,message:text,skills:selectedSkills.map(s=>({id:s.id,name:s.name,category:s.category,description:s.description,prompt:s.prompt})),references:refs,referenceContext:message.referencesInherited?'continued':'explicit'})});const result=await response.json();if(!response.ok)throw Error(result.error||'Agent 请求失败');conversation.backendId=result.conversationId;message.answer=sanitizeAgentReply(text, result.reply);message.tasks=result.tasks;message.toolTrace=result.toolTrace;message.reasoningSummary=result.reasoningSummary;attachGenerationReceipts(conversation.messages,message,result.generationTasks,models);}catch(e){message.error=(/failed to fetch|networkerror|load failed/i.test(e.message||'')?'与后端的连接中断，暂未取得任务结果。请先查看任务记录，确认状态后再重试，避免重复提交。':/not implemented/i.test(e.message||'')?'当前模型暂不支持该调用方式，已可切换其他 Agent 模型或重试':(e.message||'连接失败，请重试'));}finally{message.pending=false;message.completedAt=Date.now();agentSending=false;$('#send-prompt').disabled=false;saveSession();renderConversation();}
};
...
renderConversation=function(...args){
 const viewport=conversationViewport(conversationLog),position=viewport.capture();
 ...
 openAtLatest=false;
 viewport.restore(position,{latest:opening});
};
renderConversation();
...
// Continuation references belong to this conversation, not the global upload tray.
function renderReferenceContinuation(){
 let row=document.getElementById('reference-continuation');
 if(!row){row=document.createElement('label');row.id='reference-continuation';row.className='conversation-meta';row.style.cssText='display:flex;align-items:center;gap:8px;padding:6px 0;white-space:normal';const check=document.createElement('input');check.type='checkbox';const text=document.createElement('span');row.append(check,text);document.querySelector('.composer-text')?.append(row);check.onchange=()=>{if(currentConversation){currentConversation.inheritReferences=check.checked;saveSession();renderReferenceContinuation();}};}
 const refs=selectConversationReferences({messages:currentConversation?.messages||[],inherit:true}).references;
 row.hidden=!currentConversation||!refs.length||$('#creation-kind').value!=='agent';
 row.style.display=row.hidden?'none':'flex';
 row.querySelector('input').checked=currentConversation?.inheritReferences!==false;
 row.querySelector('span').textContent='沿用本会话参考：'+refs.map(r=>'@'+(r.reference||r.file?.name||'素材')).join('、')+'（本轮 @ 优先）';
}
const renderBeforeReferenceContinuation=renderConversation;
renderConversation=function(...args){renderBeforeReferenceContinuation.apply(this,args);renderReferenceContinuation();};
$('#creation-kind').addEventListener('change',renderReferenceContinuation);
renderReferenceContinuation();
